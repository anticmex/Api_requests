...
class MainUser:
    ...
    def __init__(self, user_id, token):
        ...
        self.base_url = 'https://api.vk.com/method/'
        self.request_friends = requests.get(
            f'{self.__main_url__("friends.get")}?user_id={self.user_id}&v=5.21',
            params=self.params)
        self.vk_user_url_get = requests.get(
            f'https://api.vk.com/method/users.get?user_ids={user_id}&fields=domain&v=5.120',
            params={'access_token': self.token})
        self.adress_url = 'https://vk.com/' + self.vk_user_url_get.json()['response'][0]['domain']
    ...
    def __and__(self, other):
        ...
        user1_friends = set(self.request_friends.json()['response']['items'])
        user2_friends = set(other.request_friends.json()['response']['items'])

        return user1_friends & user2_friends

    def __str__(self):
        return self.adress_url


token = input("Введите токен для дальнейшей работы: ")

user1 = MainUser('175798154', token)
user2 = MainUser('16205368', token)
